...
class SearchApiMetadataSourcePlugin(
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # Successful search responses keyed by their normalized parameters,
        # grouped by query type. Repeated searches reuse cached results, and a
        # later search with no valid query or filters falls back to the most
        # recent cached response for that type instead of replacing existing
        # candidates with an empty result. Insertion order of each inner dict
        # tracks the most recent search.
        self._search_cache: dict[
            QueryType, dict[tuple, tuple[SearchParams, Sequence[R]]]
        ] = {"album": {}, "track": {}}
    ...
    def _normalize_search_filters(
        ...
        """Strip filter values and discard filters without a usable value.

        Whitespace-only entries carry no search intent, so they are removed to
        avoid triggering pointless provider requests.
        """
        return {
            key: value.strip()
    ...
    @staticmethod
    def _search_cache_key(params: SearchParams) -> tuple:
        """Build a hashable cache key from normalized search parameters."""
        return (
            params.query,
            tuple(sorted(params.filters.items())),
            params.limit,
        )

    def _latest_cached_results(self, query_type: QueryType) -> Sequence[R]:
        """Return the most recent successful response for a query type."""
        cache = self._search_cache[query_type]
        if not cache:
            return ()
        return next(reversed(cache.values()))[1]
    ...
    def _search_api(
        ...
        # Normalize the search conditions: blank text and empty filter values
        # carry no search intent.
        query = query.strip()
        filters = self._normalize_search_filters(filters)
        ...
        if cache_key in cache:
            self._log.debug(
                "Reusing cached search results for '{}' with {}",
                query,
                filters,
            )
        ...
        if not query and not filters:
            # No valid search conditions. Keep any candidates already resolved
            # for this query type instead of firing off an empty request whose
            # empty response would overwrite them.
            cached_results = self._latest_cached_results(query_type)
            self._log.debug(
                "Skipping search with empty query and filters "
                "({} cached result(s) kept)",
                len(cached_results),
            )
        ...
        # Only successful responses are cached; a failure either propagates via
        # raise_on_error or falls back to an empty result above without poisoning
        # the cache. Reinsertion is unnecessary: an existing key returns early.
        cache[cache_key] = (params, response_data)
